...
DefinitionBlock("", "SSDT", 2, "hack", "_PNLF", 0)
{
    ...
    Device(BKLT)
    {
        ...
        Name (LMAX, 0)
        Name (FBTP, 0)
        If (\_SB.PCI0.IGPU.PNLF._UID == 14) 
        { 
            LMAX = 0x710
            FBTP = 1 
        }
        ElseIf (\_SB.PCI0.IGPU.PNLF._UID == 15) 
        { 
            LMAX = 0xad9
            FBTP = 2 
        }
        ElseIf (\_SB.PCI0.IGPU.PNLF._UID == 16) 
        { 
            LMAX = 0x56c
            FBTP = 2
        }
        ElseIf (\_SB.PCI0.IGPU.PNLF._UID == 17) 
        { 
            LMAX = 0x07a1
            FBTP = 0 
        }
        ElseIf (\_SB.PCI0.IGPU.PNLF._UID == 18) 
        { 
            LMAX = 0x1499
            FBTP = 0
        }
        If (\_SB.PCI0.IGPU.PNLF._UID == 19) 
        { 
            LMAX = 0xffff
            FBTP = 3
        }
    ...
    Device(_SB.PCI0.IGPU.PNLF)
    {
        ...
        Method(INI1, 1)
        {
            ...
            {
                Local5 = 0xC0000000
                If (CondRefOf(\BKLT.LEVW)) { If (Ones != \BKLT.LEVW) { Local5 = \BKLT.LEVW } }
                ^LEVW = Local5
            }
            // from step 2 above (you may need 1 instead)
            If (4 & Arg0)
            {
                If (CondRefOf(\BKLT.GRAN)) { ^GRAN = \BKLT.GRAN }
                Else { ^GRAN = 0 }
            }
        }
        ...
        Method(_INI)
        {
            ...
            Local4 = 1
            If (CondRefOf(\BKLT.BLGT)) { Local4 = \BKLT.BLGT }
            If (!(1 & Local4)) { Return }

            // Adjustment required when using AppleBacklight.kext
            Local0 = ^GDID
            Local2 = Ones
            If (CondRefOf(\BKLT.LMAX)) { Local2 = \BKLT.LMAX }
            // Determine framebuffer type (for PWM register layout)
            Local3 = 0
            If (CondRefOf(\BKLT.FBTP)) { Local3 = \BKLT.FBTP }
